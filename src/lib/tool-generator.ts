...
function toolsFor(category: SiteCategory, name: string): WebMCPTool[] {
  switch (category) {
    ...
    case "ecommerce":
      return [
        mk({ name: "list_departments", label: "List departments and categories", description: `Declarative: List the ${name} departments and category tree an agent can browse, with the category handle used by every other tool. Imperative: Requires nothing; optional parent_category_id.`, type: "content", method: "GET", path: "/api/collections", safety: "safe", inputSchema: { type: "object", properties: { parent_category_id: { type: "string", description: "Narrow to the children of one category" } }, required: [] }, examplePrompt: `What can I shop for on ${name}?` }),
        mk({ name: "search_products", label: "Search the catalog", description: `Declarative: Search the ${name} catalog in natural language and open the results page, with facets for category, price, brand, rating, and availability. Imperative: Requires query; optional category_id, min_price, max_price, brand, in_stock_only, sort, page.`, type: "search", method: "GET", path: "/api/search", safety: "safe", inputSchema: { type: "object", properties: { query: { type: "string", description: "What the shopper is looking for" }, category_id: { type: "string" }, min_price: { type: "number" }, max_price: { type: "number" }, brand: { type: "string" }, in_stock_only: { type: "boolean" }, sort: { type: "string", enum: ["relevance", "price_asc", "price_desc", "rating", "newest"] }, page: { type: "number" } }, required: ["query"] }, examplePrompt: `Find a quiet cordless vacuum under $250 on ${name}` }),
        mk({ name: "get_product", label: "Get a product", description: "Declarative: Get full detail for one product: price, promotions, ratings, specs, images, and the variant options that must be chosen before adding to cart. Imperative: Requires product_id.", type: "content", method: "GET", path: "/api/products/{product_id}", safety: "safe", inputSchema: { type: "object", properties: { product_id: { type: "string", description: "Product handle or SKU from search results" } }, required: ["product_id"] }, examplePrompt: "Tell me more about that second one" }),
        mk({ name: "list_product_variants", label: "List variants", description: "Declarative: List every purchasable variant of a product (size, color, pack size, subscription vs one-time) with its own price and stock state. Imperative: Requires product_id.", type: "content", method: "GET", path: "/api/products/{product_id}/variants", safety: "safe", inputSchema: { type: "object", properties: { product_id: { type: "string" } }, required: ["product_id"] }, examplePrompt: "Which sizes and colors does it come in?" }),
        mk({ name: "check_availability", label: "Check stock and pickup", description: "Declarative: Check live availability for a variant, both for shipping and for pickup at nearby stores. Imperative: Requires variant_id; optional postal_code, quantity.", type: "content", method: "GET", path: "/api/inventory", safety: "safe", inputSchema: { type: "object", properties: { variant_id: { type: "string" }, postal_code: { type: "string", description: "For store pickup availability" }, quantity: { type: "number" } }, required: ["variant_id"] }, examplePrompt: "Can I pick it up near 60614 today?" }),
        mk({ name: "compare_products", label: "Compare products", description: "Declarative: Put two to four products side by side on price, key specs, ratings, and availability. Imperative: Requires product_ids.", type: "content", method: "GET", path: "/api/compare", safety: "safe", inputSchema: { type: "object", properties: { product_ids: { type: "array", description: "Two to four product ids" } }, required: ["product_ids"] }, examplePrompt: "Compare those three" }),
        mk({ name: "get_cart", label: "Get the cart", description: "Declarative: Read the current cart: line items, quantities, item subtotals, applied promotions, and the cart_id used by the write tools. Imperative: Requires nothing; optional cart_id.", type: "content", method: "GET", path: "/api/cart", safety: "safe", inputSchema: { type: "object", properties: { cart_id: { type: "string" } }, required: [] }, examplePrompt: "What's in my basket?" }),
        mk({ name: "add_to_cart", label: "Add to cart", description: "Declarative: Add a chosen variant to the cart and return the updated cart with the new subtotal. Imperative: Requires variant_id and quantity; optional cart_id, fulfillment (ship or pickup), store_id.", type: "transaction", method: "POST", path: "/api/cart/items", safety: "confirmation_required", inputSchema: { type: "object", properties: { variant_id: { type: "string" }, quantity: { type: "number" }, cart_id: { type: "string" }, fulfillment: { type: "string", enum: ["ship", "pickup"] }, store_id: { type: "string" } }, required: ["variant_id", "quantity"] }, examplePrompt: "Add two of the medium, pickup at the nearest store" }),
        mk({ name: "update_cart_item", label: "Update or remove a cart item", description: "Declarative: Change the quantity of a cart line, or remove it by setting quantity to zero. Imperative: Requires line_item_id and quantity; optional cart_id.", type: "transaction", method: "POST", path: "/api/cart/items/{line_item_id}", safety: "confirmation_required", inputSchema: { type: "object", properties: { line_item_id: { type: "string" }, quantity: { type: "number", description: "0 removes the line" }, cart_id: { type: "string" } }, required: ["line_item_id", "quantity"] }, examplePrompt: "Drop that down to one" }),
        mk({ name: "apply_promo_code", label: "Apply a promo code", description: "Declarative: Apply a discount or loyalty code to the cart and report whether it was accepted and what it saved. Imperative: Requires code; optional cart_id.", type: "transaction", method: "POST", path: "/api/cart/discounts", safety: "confirmation_required", inputSchema: { type: "object", properties: { code: { type: "string" }, cart_id: { type: "string" } }, required: ["code"] }, examplePrompt: "Try code SPRING15" }),
        mk({ name: "quote_order_total", label: "Quote the order total", description: "Declarative: Price the cart before any commitment: shipping options, delivery estimates, taxes, discounts, and the final total. Imperative: Requires postal_code; optional cart_id, country, shipping_method.", type: "content", method: "GET", path: "/api/cart/estimate", safety: "safe", inputSchema: { type: "object", properties: { postal_code: { type: "string" }, country: { type: "string" }, shipping_method: { type: "string" }, cart_id: { type: "string" } }, required: ["postal_code"] }, examplePrompt: "What's the total delivered to 94110, and when would it arrive?" }),
        mk({ name: "start_checkout", label: "Start checkout", description: "Declarative: Open the hosted checkout for the current cart and return its URL so the shopper enters payment details themselves. Imperative: Requires nothing; optional cart_id, email.", type: "transaction", method: "POST", path: "/api/checkout", safety: "sensitive", inputSchema: { type: "object", properties: { cart_id: { type: "string" }, email: { type: "string", format: "email" } }, required: [] }, examplePrompt: "Take me to checkout" }),
        mk({ name: "track_order", label: "Track an order", description: "Declarative: Look up an order's fulfillment status, shipments, and tracking links. Imperative: Requires order_id and email.", type: "account", method: "GET", path: "/api/orders/{order_id}", safety: "sensitive", inputSchema: { type: "object", properties: { order_id: { type: "string" }, email: { type: "string", format: "email" } }, required: ["order_id", "email"] }, examplePrompt: "Where is order 84421?" }),
        mk({ name: "start_return", label: "Start a return or exchange", description: "Declarative: Open a return or exchange for delivered items and return the label or drop-off instructions. Imperative: Requires order_id, line_item_id, and reason; optional resolution (refund or exchange).", type: "support", method: "POST", path: "/api/returns", safety: "sensitive", inputSchema: { type: "object", properties: { order_id: { type: "string" }, line_item_id: { type: "string" }, reason: { type: "string" }, resolution: { type: "string", enum: ["refund", "exchange"] } }, required: ["order_id", "line_item_id", "reason"] }, examplePrompt: "Return the jacket from order 84421, wrong size" }),
      ];
    ...
    case "marketplace":
      return [
        mk({ name: "list_marketplace_categories", label: "List categories", description: `Declarative: List the ${name} categories an agent can browse, with the category handle used by search. Imperative: Requires nothing; optional parent_category_id.`, type: "content", method: "GET", path: "/api/categories", safety: "safe", inputSchema: { type: "object", properties: { parent_category_id: { type: "string" } }, required: [] }, examplePrompt: `What kinds of listings are on ${name}?` }),
        mk({ name: "search_listings", label: "Search listings", description: "Declarative: Search seller listings in natural language with price, condition, location, shipping, and seller-rating facets. Imperative: Requires query; optional category_id, min_price, max_price, condition, location, ships_to, sort, page.", type: "search", method: "GET", path: "/api/search", safety: "safe", inputSchema: { type: "object", properties: { query: { type: "string" }, category_id: { type: "string" }, min_price: { type: "number" }, max_price: { type: "number" }, condition: { type: "string", enum: ["new", "like_new", "used", "refurbished"] }, location: { type: "string" }, ships_to: { type: "string" }, sort: { type: "string", enum: ["relevance", "price_asc", "price_desc", "newest", "ending_soon"] }, page: { type: "number" } }, required: ["query"] }, examplePrompt: "Find a used road bike under $600 within 20 miles" }),
        mk({ name: "get_listing", label: "Get a listing", description: "Declarative: Get one listing in full: price or current bid, condition notes, photos, shipping options, return policy, and the seller handle. Imperative: Requires listing_id.", type: "content", method: "GET", path: "/api/listings/{listing_id}", safety: "safe", inputSchema: { type: "object", properties: { listing_id: { type: "string" } }, required: ["listing_id"] }, examplePrompt: "Show me the details on that one" }),
        mk({ name: "get_seller_profile", label: "Get seller profile", description: "Declarative: Read a seller's rating, review count, dispute history, shipping speed, and other active listings. Imperative: Requires seller_id.", type: "content", method: "GET", path: "/api/sellers/{seller_id}", safety: "safe", inputSchema: { type: "object", properties: { seller_id: { type: "string" } }, required: ["seller_id"] }, examplePrompt: "Is this seller trustworthy?" }),
        mk({ name: "quote_shipping", label: "Quote shipping and total", description: "Declarative: Price a listing delivered: shipping options, delivery window, import fees where relevant, and the all-in total. Imperative: Requires listing_id and postal_code; optional quantity, country.", type: "content", method: "GET", path: "/api/listings/{listing_id}/shipping", safety: "safe", inputSchema: { type: "object", properties: { listing_id: { type: "string" }, postal_code: { type: "string" }, country: { type: "string" }, quantity: { type: "number" } }, required: ["listing_id", "postal_code"] }, examplePrompt: "What would it cost shipped to 30303?" }),
        mk({ name: "message_seller", label: "Message the seller", description: "Declarative: Send a question to the seller through the marketplace inbox and return the thread id. Imperative: Requires listing_id and body.", type: "support", method: "POST", path: "/api/messages", safety: "confirmation_required", inputSchema: { type: "object", properties: { listing_id: { type: "string" }, body: { type: "string" } }, required: ["listing_id", "body"] }, examplePrompt: "Ask if they'd take $500" }),
        mk({ name: "make_offer", label: "Make an offer", description: "Declarative: Submit a binding offer on a listing that accepts offers, and report whether it was accepted, countered, or declined. Imperative: Requires listing_id and amount; optional message, expires_in_hours.", type: "transaction", method: "POST", path: "/api/listings/{listing_id}/offers", safety: "sensitive", inputSchema: { type: "object", properties: { listing_id: { type: "string" }, amount: { type: "number" }, message: { type: "string" }, expires_in_hours: { type: "number" } }, required: ["listing_id", "amount"] }, examplePrompt: "Offer $520 with free local pickup" }),
        mk({ name: "add_listing_to_cart", label: "Add a listing to cart", description: "Declarative: Add a fixed-price listing to the cart and return the updated cart with per-seller subtotals. Imperative: Requires listing_id and quantity; optional cart_id.", type: "transaction", method: "POST", path: "/api/cart/items", safety: "confirmation_required", inputSchema: { type: "object", properties: { listing_id: { type: "string" }, quantity: { type: "number" }, cart_id: { type: "string" } }, required: ["listing_id", "quantity"] }, examplePrompt: "Add it to my cart" }),
        mk({ name: "start_checkout", label: "Start checkout", description: "Declarative: Open the marketplace's protected checkout for the cart and return its URL so the buyer enters payment details themselves. Imperative: Requires nothing; optional cart_id.", type: "transaction", method: "POST", path: "/api/checkout", safety: "sensitive", inputSchema: { type: "object", properties: { cart_id: { type: "string" } }, required: [] }, examplePrompt: "Check out with buyer protection" }),
        mk({ name: "save_search_alert", label: "Save a search alert", description: "Declarative: Save a search so the buyer is alerted when matching listings appear. Imperative: Requires query; optional max_price, category_id, location.", type: "account", method: "POST", path: "/api/saved-searches", safety: "safe", inputSchema: { type: "object", properties: { query: { type: "string" }, max_price: { type: "number" }, category_id: { type: "string" }, location: { type: "string" } }, required: ["query"] }, examplePrompt: "Alert me when one under $500 is listed" }),
        mk({ name: "watch_listing", label: "Watch a listing", description: "Declarative: Add a listing to the buyer's watchlist or favorites. Imperative: Requires listing_id.", type: "account", method: "POST", path: "/api/watchlist", safety: "safe", inputSchema: { type: "object", properties: { listing_id: { type: "string" } }, required: ["listing_id"] }, examplePrompt: "Save this one for later" }),
        mk({ name: "track_order", label: "Track a purchase", description: "Declarative: Look up a purchase's payment, shipping, and dispute status with tracking links. Imperative: Requires order_id; optional email.", type: "account", method: "GET", path: "/api/orders/{order_id}", safety: "sensitive", inputSchema: { type: "object", properties: { order_id: { type: "string" }, email: { type: "string", format: "email" } }, required: ["order_id"] }, examplePrompt: "Has my bike shipped yet?" }),
        mk({ name: "create_listing_draft", label: "Draft a listing to sell", description: "Declarative: Create a draft listing on the seller side with title, category, condition, price, and photos, returning a draft id to review before publishing. Imperative: Requires title, category_id, condition, and price; optional description, photo_urls, quantity.", type: "form", method: "POST", path: "/api/listings/drafts", safety: "sensitive", inputSchema: { type: "object", properties: { title: { type: "string" }, category_id: { type: "string" }, condition: { type: "string", enum: ["new", "like_new", "used", "refurbished"] }, price: { type: "number" }, description: { type: "string" }, photo_urls: { type: "array" }, quantity: { type: "number" } }, required: ["title", "category_id", "condition", "price"] }, examplePrompt: "List my old road bike for $600" }),
      ];
